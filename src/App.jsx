...
// --- ARQUIVOS (agora na pasta public) ---
...
// --- ÍCONES (mantidos como SVG inline) ---
const Icons = {
...
const PulseButton = ({ text, href, onClick, className = "", variant = "orange" }) => {
    ...
    const handleClick = (e) => {
        if (href && href.includes("SEU-LINK-AQUI")) { e.preventDefault(); alert("Atenção: Link de checkout não configurado."); } 
...
const LeadCaptureForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsSending(true);
        try {
            const response = await fetch("https://formspree.io/f/xpqwvvea", { 
                method: "POST", 
                headers: { 'Content-Type': 'application/json' }, 
                body: JSON.stringify({ email, origem: "Site Nano Franquia - Lead" }) 
            });
            if (response.ok) { setSubmitted(true); setEmail(""); } 
            else { alert("Ocorreu um erro ao enviar. Por favor, tente novamente."); }
        } catch (error) { alert("Erro de conexão. Verifique sua internet."); } 
        finally { setIsSending(false); }
    };

    if (submitted) return (
        <div className="mb-8 p-4 bg-green-50 rounded-lg border border-green-200 text-center fade-in">
            <p className="text-green-800 font-bold flex items-center justify-center gap-2">
                <Icons.CheckCircle size={20} /> Sucesso! Enviamos os detalhes para seu e-mail.
            </p>
        </div>
    );
    ...
    return (
        <div className="mb-8 p-6 bg-blue-50 rounded-xl border border-blue-100 text-left">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2">
                ...
                <button type="submit" disabled={isSending} className={`bg-blue-700 hover:bg-blue-800 text-white px-6 py-4 rounded font-bold transition flex items-center justify-center text-lg ${isSending ? 'opacity-70 cursor-not-allowed' : ''}`}>
...
const LoginModal = ({ isOpen, onClose }) => {
    ...
    const handleLogin = (e) => { 
        e.preventDefault(); 
        setError(""); 
        setTimeout(() => setError("Usuário não encontrado ou senha incorreta. Se você já comprou, verifique seu e-mail de boas-vindas."), 800); 
    };
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 fade-in backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-md p-8 relative shadow-2xl">
                ...
                <div className="text-center mb-8">
                    <div className="w-16 h-16 bg-blue-100 text-blue-900 rounded-full flex items-center justify-center mx-auto mb-4"><Icons.User size={32} /></div>
                    <h3 className="text-2xl font-bold text-gray-900">Acesso ao Portal</h3>
                    <p className="text-gray-500 text-sm">Área exclusiva para técnicos certificados</p>
                </div>
                <form onSubmit={handleLogin} className="space-y-4">
                    <div>
                        <label className="block text-xs font-bold text-gray-500 uppercase mb-1">E-mail de Acesso</label>
                        <input type="email" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="seu@email.com" value={email} onChange={(e) => setEmail(e.target.value)} required />
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Senha</label>
                        <input type="password" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required />
                    </div>
                ...
                <div className="mt-6 text-center border-t border-gray-100 pt-4">
                    <p className="text-gray-600 text-sm">Ainda não é um técnico?</p>
                    <button onClick={() => { onClose(); document.getElementById('oferta').scrollIntoView({ behavior: 'smooth' }); }} className="text-orange-600 font-bold hover:underline mt-1">Torne-se um Fundador Agora</button>
                </div>
            </div>
        </div>
    );
    ...
    const handleSubmit = async (e) => { 
        ...
        try { 
            const response = await fetch("https://formspree.io/f/xpqwvvea", { 
                method: "POST", 
                headers: { 'Content-Type': 'application/json' }, 
                body: JSON.stringify({ ...formData, origem: "Cadastro de Técnico" }) 
            }); 
            if (response.ok) { alert("Pré-cadastro realizado!"); onClose(); setFormData({ nome: "", cidade: "", whatsapp: "", experiencia: "Sou iniciante" }); } 
    ...
    const handleSubmit = async (e) => { 
        ...
        try { 
            const response = await fetch("https://formspree.io/f/xpqwvvea", { 
                method: "POST", 
                headers: { 'Content-Type': 'application/json' }, 
                body: JSON.stringify({ ...formData, origem: "Cadastro de Salão" }) 
            }); 
            if (response.ok) { alert("Cadastro realizado!"); onClose(); setFormData({ nomeSalao: "", responsavel: "", cidade: "", whatsapp: "" }); } 
            else { alert("Erro ao enviar."); } 
        } catch (error) { alert("Erro de conexão."); } 
        finally { setIsSending(false); } 
    };
                ...
                <div className="flex flex-col md:flex-row items-center gap-12">
                    ...
                    <div className="w-full md:w-1/2 text-left space-y-8">
                        ...
                        <div className="space-y-6 text-gray-700 text-lg md:text-xl">
                            <p className="leading-relaxed">Em 25 anos de bancada e 17 milhões faturados, meu problema nunca foi falta de cliente. <strong>Meu problema era encontrar técnicos.</strong></p>
                            ...
                            <p className="font-bold text-blue-900 text-xl pt-2">A ficha caiu. O mercado precisa de Especialistas, não de curiosos.</p>
                            <p className="leading-relaxed">Criei a Hometech para resolver essas três dores: te dar a qualificação real, o acesso às peças com preço de fábrica e, o principal:</p>
